\documentclass[]{mcec-cv}
 
\begin{document}
%----- SIDEBAR -----%

\profilepic{monk.jpg} % path of profile pic
\cvname{Adrian Monk} % name
\cvjobtitle{Private police homicide consultant} % actual job position
\cvdate{17 October 1959} % date of birth
\cvaddress{San Francisco, California} % address
\cvnumberphone{+00 123 4567890} % telphone number
\cvmail{adrian@monk.com} % e-mail address
\cvsitename{https://en.wikipedia.org} % personal site short address
\cvsiteaddress{https://en.wikipedia.org/wiki/Adrian_Monk}

\aboutme{Monk (Tony Shalhoub) is of Welsh ancestry and was born in 1959 in the fictitious Marin County town of Tewksbury, to parents Agnes and Jack Monk. Jack abandoned the family when Adrian was eight years old. Adrian has an agoraphobic brother named Ambrose (John Turturro), from whom he was estranged for seven years following Trudy's death. \\Throughout the series, Adrian mourns his wife Trudy (Melora Hardin/Stellina Rusich), who was killed by a car bomb he believes was meant for him on December 14, 1997.}

\languages{{English/Native or bilingual proficiency},{Welsh/Elementary proficiency}}
\mcecskills{{Detective},{Memory},{Perspicacity},{Detective},{Perspicacity},{Detective},{Memory},{Perspicacity}}

% skill bar section, each skill must have a value between 0 an 6 (float)
%\skills{{pursuer of rabbits/6},{good manners/4},{outgoing/3},{polite/4},{Java/1}}
%\skillstext{{lovely/4},{narcissistic/3}}

\makeprofile

%----- body -----%

\maketitlename

% mcecllist (long list): [5]
% 	(1) job position / title school
%	(2) company name / school course
% 	(3) date (4) location (5) description

% mcecnlist (normal list): [3]
% 	(1) title certificate (2) code (3) date 

% mcecslist (short list): [2]
% 	(1) date (2) title

\section{Experience}

\begin{mcecllist}
\litem
{Main Character}
{Monk (TV Series)}
{July 12, 2002 - December 4, 2009}
{United States}
{Monk is an American comedy-drama detective mystery television series created by Andy Breckman and starring Tony Shalhoub as the eponymous character, Adrian Monk. It is primarily a police procedural series, and also exhibits comic and dramatic tones in its exploration of the main characters' personal lives. The series was produced by Mandeville Films and Touchstone Television in association with Universal Television. The series debuted on July 12, 2002, on USA Network. It continued for eight seasons, with the final season concluding on December 4, 2009. The series held the record for the most-watched scripted drama episode in cable television history from 2009 through 2012}

\litem
{Mention}
{Veronica Mars (TV Series) | President Evil}
{October 31, 2006}
{United States}
{In episode 5 of third season of \emph{Veronica Mars} TV Series, Keith Mars, father of main character Veronica, is accused to impersonating the detective Adrian Monk.}

\litem
{Mention}
{The Simpson (Animated sitcom) | Mona Leaves-a}
{May 11, 2008}
{United States}
{In episode 19 of nineteenth season of \emph{The Simpsons} animated sitcom, Adrian Monk is mentioned.}

\end{mcecllist}

\section{Education}

\begin{mcecllist}

\litem
{University of San Francisco State University}
{Criminal Justice Studies}
{1986}
{San Francisco, California}
{Positioned within a rich urban atmosphere, the multidisciplinary Department of Criminal Justice Studies explores the connections between law, crime and justice. The curriculum emphasizes critical thinking about criminal justice systems, the administration of justice, legal studies, and the political economy.}

\litem
{University of Berkeley, California}
{Letters \& Science}
{1981}
{Berkeley, California}
{The intellectual heart of UC Berkeley, the College of Letters and Science (L\&S) is the largest of the university's 14 colleges and schools and the most prestigious teaching and research unit in the UC system. L\&S encompasses more than half of the campus's faculty, three-quarters of its undergraduate students, and half of its Ph.D. candidates.}

\end{mcecllist}

\section{Certifications}

\begin{mcecnlist}

\nitem
{Fear of germs}
{.Life, Licence 9876}
{1990}
    
\end{mcecnlist}

\section{awards}

\begin{mcecslist}

\sitem {2006} {Primetime Emmy Award for Outstanding Lead Actor in a Comedy Series}
\sitem {2005} {Screen Actors Guild Award for Outstanding Performance by a Male Actor in a Comedy Series}
\sitem {2005} {Primetime Emmy Award for Outstanding Lead Actor in a Comedy Series}
\sitem {2004} {Screen Actors Guild Award for Outstanding Performance by a Male Actor in a Comedy Series}
\sitem {2003} {Primetime Emmy Award for Outstanding Lead Actor in a Comedy Series}
\sitem {2003} {Golden Globe Award for Best Actor – Television Series Musical or Comedy}

\end{mcecslist}

\section{Other information}
Monk has 312 phobias. The strongest of these phobias are: germs, dentists, sharp or pointed objects, milk, vomiting, death and dead things, snakes, crowds, heights, fear, mushrooms, and small spaces. In addition, new phobias develop at seemingly random intervals, such as a temporary fear of blankets.\\
Due to his overpowering fear of germs, Monk refuses to touch door handles and other common objects with his bare hands, avoids contact with anything dirty, and always uses sanitary wipes after human contact, including basic handshakes. He is also unable to eat food that other people have touched.

\section{Interests}

%\framebox[1.05\width]{Guess I'm framed now!}
\largeflag{clarinet}
\largeflag{murder}
\largeflag{crime}
\largeflag{homicide}



\end{document} 